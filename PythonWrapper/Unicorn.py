...
class Unicorn:
    ...
    def GetAvailableDevices(rescan:bool)-> tuple[list[str], UnicornReturnStatus]:
        ...
        output = UnicornReturnStatus(unicorn.lib.UNICORN_GetAvailableDevices(None, ctypes.byref(DeviceCount), rescan))
        print(output)
    ...
    def StartAcquisition(handle:int, testSignalEnabled:bool)->UnicornReturnStatus:
        testSignalEnabled = 1 if testSignalEnabled else 0
        return UnicornReturnStatus(unicorn.lib.UNICORN_StartAcquisition(handle, testSignalEnabled))
    
    def StopAcquisition(handle:int)->UnicornReturnStatus:
        return UnicornReturnStatus(unicorn.lib.UNICORN_StopAcquisition(handle))
    
    def SetConfiguration(handle:int, configuration:UnicornAmplifierConfiguration)->UnicornReturnStatus:
        ...
        return UnicornReturnStatus(unicorn.lib.UNICORN_SetConfiguration(handle, ctypes.byref(configurationCtypes)))

    def GetConfiguration(handle:int)->tuple[UnicornAmplifierConfiguration,UnicornReturnStatus]:
        configuration = UnicornAmplifierConfiguration([UnicornAmplifierChannel("", "", (0, 0), False) for i in range(unicorn.UNICORN_TOTAL_CHANNELS_COUNT)])
        configurationCtypes = unicorn.UNICORN_AMPLIFIER_CONFIGURATION()
        result = UnicornReturnStatus(unicorn.lib.UNICORN_GetConfiguration(handle, ctypes.byref(configurationCtypes)))
    ...
    def GetData(handle:int,ScanCount:int, OutputLength:int)-> tuple[list[float], UnicornReturnStatus]:
        Destination = (ctypes.c_float * OutputLength)()
        output = UnicornReturnStatus(unicorn.lib.UNICORN_GetData(handle,ScanCount, Destination, OutputLength))
    ...
    def GetNumberOfAcquiredChannels(handle:int)-> tuple[int, UnicornReturnStatus]:
        ...
        output = UnicornReturnStatus(unicorn.lib.UNICORN_GetNumberOfAcquiredChannels(handle, ctypes.byref(NumberOfAcquiredChannels)))
    ...
    def GetChannelIndex(handle:int,ChannelName:str) -> tuple[int,UnicornReturnStatus]:
        ...
        output = UnicornReturnStatus(unicorn.lib.UNICORN_GetChannelIndex(handle,ChannelName.encode(), ctypes.byref(ChannelIndex)))
        return ChannelIndex.value, output
    
    def GetDeviceInformation(handle:int)-> tuple[UnicornDeviceInformation, UnicornReturnStatus]:
        DeviceInformation = unicorn.UNICORN_DEVICE_INFORMATION()
        output = UnicornReturnStatus(unicorn.lib.UNICORN_GetDeviceInformation(handle, ctypes.byref(DeviceInformation)))
        return UnicornDeviceInformation(DeviceInformation.numberOfEegChannels, DeviceInformation.serial.decode(), DeviceInformation.firmwareVersion.decode(), DeviceInformation.deviceVersion.decode(), DeviceInformation.pcbVersion, DeviceInformation.enclosureVersion), output
    
    def SetDigitalOutput(handle:int)-> tuple[int, UnicornReturnStatus]:
        DigitalOutput = ctypes.c_uint8()
        output = UnicornReturnStatus(unicorn.lib.UNICORN_SetDigitalOutputs(handle, ctypes.byref(DigitalOutput)))
        return DigitalOutput.value, output

if __name__ == "__main__":
    output = Unicorn.OpenDevice("UN-2021.12.19")
    print(output)
    try:
        ChannelConfig = Unicorn.GetConfiguration(output[1])
        print(ChannelConfig)
        startOutput = Unicorn.StartAcquisition(output[1], True)
        print(startOutput)
        numberOfGetDataCalls = 10*unicorn.UNICORN_SAMPLING_RATE
        for i in range(numberOfGetDataCalls):
            getDataOutput = Unicorn.GetData(output[1],1,len(ChannelConfig[0].channels))
            print(getDataOutput)
        stopOutput = Unicorn.StopAcquisition(output[1])
        print(stopOutput)
    except Exception as e:
        print(e)
    finally:
        close  = Unicorn.CloseDevice(output[0])
        print(close)
